...
class AttributeBasedClassifierTest(BaseTestCase):
  def __repr__(self):
    return "AttributeBasedClassifierTest"

  def setUp(self):
    self.log.info("Setting up the engine...")

    template_path = "https://github.com/apache/incubator-predictionio-template-attribute-based-classifier"
    engine_json_path = pjoin(
        self.test_context.data_directory,
        "templates/attribute-based-classifier/engine.json")
    ...
    self.training_data_path = pjoin(
        self.test_context.data_directory,
        "templates/attribute-based-classifier/data.txt")
