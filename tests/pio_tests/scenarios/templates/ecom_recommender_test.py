...
class EcomRecommenderTest(BaseTestCase):
  def __repr__(self):
    return "EcomRecommenderTest"

  def setUp(self):
    self.log.info("Setting up the engine...")

    template_path = "https://github.com/apache/incubator-predictionio-template-ecom-recommender"

    engine_json_path = pjoin(
        self.test_context.data_directory,
        "templates/ecom-recommender/engine.json")
    ...
    self.training_data_path = pjoin(
        self.test_context.data_directory,
        "templates/ecom-recommender/data.json")
  ...
  def runTest(self):
    ...
    self.log.info("Unsetting constraint \"unavailableItems\"...")
    self.app.send_event({
        "event" : "$set",
        "entityType" : "constraint",
        "entityId" : "unavailableItems",
