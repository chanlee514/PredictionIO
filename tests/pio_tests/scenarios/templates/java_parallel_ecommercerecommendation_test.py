...
class JavaParallelEcommercerecommendationTest(BaseTestCase):

	def setUp(self):
		self.log.info("Setting up the engine")
		template_path = "https://github.com/chanlee514/template-java-parallel-ecommercerecommendation"
		engine_json_path = pjoin(
		    self.test_context.data_directory,
		    "java_parallel_ecommercerecommendation_test/engine.json")
		...
		# Share training data with scala ecommercerecommendation engine
	...
	def runTest(self):
		...
		r = self.app.query({ "userEntityId": "u1", "number": 4 })
		...
		for i in r.json()['itemScores']:
			self.assertNotIn(i['itemEntityId'], unavailableItems)
		...
		r = self.app.query({
				...
				'categories': ['c3', 'c4'] })
		for i in r.json()['itemScores']:
			self.assertIn(i['itemEntityId'], possibleItems)
		...
		r = self.app.query({
				...
				'whitelist': whiteList })
		for i in r.json()['itemScores']:
			self.assertIn(i['itemEntityId'], whiteList)
		...
		r = self.app.query({
				...
				'blacklist': blackList })
		for i in r.json()['itemScores']:
			self.assertNotIn(i['itemEntityId'], blackList)
