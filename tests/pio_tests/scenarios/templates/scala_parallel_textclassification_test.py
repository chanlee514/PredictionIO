...
class ScalaParallelTextclassificationTest(BaseTestCase):

  def setUp(self):
    self.log.info("Setting up the engine")
    template_path = "https://github.com/chanlee514/template-scala-parallel-textclassification"
    engine_json_path = pjoin(
      self.test_context.data_directory, "scala_parallel_textclassification_test/engine.json")
  ...
  def runTest(self):
    ...
    engine_path = pjoin(self.test_context.engine_directory,
        "template-scala-parallel-textclassification/data")
    self.app.import_events_from_file(
        "{}/emails.json".format(engine_path))
    self.app.import_events_from_file(
        "{}/stopwords.json".format(engine_path))
